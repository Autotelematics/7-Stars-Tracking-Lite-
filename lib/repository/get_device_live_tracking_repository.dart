...
class GetDeviceLiveTrackingRepository {
  Future<GetDeviceLiveTrackingModel?> getDeviceLiveTrackingApi({required int deviceId,required BuildContext dialogContext}) async {
    String? userApiHashKey = await UserSessions.getUserApiHash();
    final headers = {"Content-Type": "application/json"};
    final url = Uri.parse(
        "${ApiEndpointUrls.baseURL}${ApiEndpointUrls.getDeviceLiveTrackingEndPoint}$userApiHashKey&id=$deviceId");
    http.Response response = await http.get(headers: headers, url);
    if (response.statusCode == 200) {
      return getDeviceLiveTrackingModelFromJson(response.body);
